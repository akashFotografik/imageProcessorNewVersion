  ...
  const handleDragEnd = useCallback((appointmentId: number, newStartTime?: string, newEndTime?: string, newUserId?: number) => {
    setAppointments(prev => prev.map(apt => {
      if (apt.id === appointmentId) {
        const updatedAppointment = { ...apt };
        
        // Update times if provided
        if (newStartTime) updatedAppointment.startTime = newStartTime;
        if (newEndTime) updatedAppointment.endTime = newEndTime;
        
        // Update user assignment if provided and different
        if (newUserId && newUserId !== apt.userId) {
          updatedAppointment.userId = newUserId;
          console.log(`Appointment "${apt.title}" reassigned from user ${apt.userId} to user ${newUserId}`);
        }
  ...
  const handleDrop = useCallback((e: React.DragEvent, userId: number) => {
    ...
    if (appointmentId) {
      ...
      if (appointment && appointment.userId !== userId) {
        console.log(`Dropping appointment ${id} onto user ${userId}`);
        handleDragEnd(id, undefined, undefined, userId);
      }
    }
    
    // Handle custom drag state
    if (dragState.appointmentId && dragState.originalPosition) {
      const originalUserId = dragState.originalPosition.userId;
      if (originalUserId !== userId) {
        console.log(`Moving appointment ${dragState.appointmentId} from user ${originalUserId} to user ${userId}`);
        handleDragEnd(dragState.appointmentId, undefined, undefined, userId);
      }
    }
  }, [dragState, appointments, handleDragEnd]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-xl overflow-hidden">
        ...
        <div className="flex max-h-screen overflow-hidden calendar-grid">
          <TimeColumn />
        ...
        <div className="p-4 bg-blue-50 border-t">
          <div className="text-sm text-blue-700">
            ...
            • <strong>Drag appointments between users to reassign</strong>
            ...
            • Visual feedback shows when dragging over different users
          </div>
        </div>
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="p-2 bg-gray-100 border-t text-xs text-gray-600">
            <strong>Debug:</strong> Drag State: {dragState.isDragging ? 'Active' : 'Inactive'} | 
            Appointments: {appointments.length} | 
            {dragState.appointmentId && ` Dragging: ${dragState.appointmentId}`}
          </div>
        )}
      </div>
